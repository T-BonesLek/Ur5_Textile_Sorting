...
def move_to_place_wool(standalone, sim):
        if standalone:
            rclpy.init()
        ...
        node.declare_parameter("position2", [0.7, -0.4, -0.1])
        node.declare_parameter("position3", [0.7, -0.4, -0.2])
...
if __name__ == "__main__":
    move_to_place_cotton(standalone=True, sim=False)
